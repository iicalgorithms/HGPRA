...
#     else:
    ...
    # 确保G是Float类型
    data.edge_index = torch.Tensor(G).float()
    return data
...
def ExtractV2E(data):
    ...
    # -------- 兼容不同数据类型 --------
    # 确保 n_x 与 num_hyperedges 是 python int，用于后续一致性检查
    if isinstance(data.n_x, torch.Tensor):
        data_nx = int(data.n_x.item())
    else:
        data_nx = int(data.n_x)

    if isinstance(data.num_hyperedges, torch.Tensor):
        data_ne = int(data.num_hyperedges.item())
    else:
        data_ne = int(data.num_hyperedges)
    ...
    num_nodes = data_nx
    num_hyperedges = data_ne
    if (num_nodes + num_hyperedges - 1) != edge_index.max().item():
        print('num_hyperedges does not match! 1 --> 自动跳过检查')
        # 不再返回 None，而是继续后续逻辑，防止上层收到 None
        # 在极端情况下可能说明输入 edge_index 已是 V2E 模式
...
def Add_Self_Loops(data):
    ...
    skip_node_lst = sorted(list(set(skip_node_lst)))

    # FIXME: need to delete the multiple self-loops
