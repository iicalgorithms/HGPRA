...
#         zeros(self.bias)
    ...
    def forward(self, x, edge_index: Adj,
        ...
        """
        # 克隆edge_index以防止原地修改导致梯度问题
        if isinstance(edge_index, Tensor) and hasattr(edge_index, 'clone'):
...
class HGNN_conv(nn.Module):
    def __init__(self, in_ft, out_ft, bias=True):
        ...
        # 添加一个可学习的缩放因子，提高训练稳定性
        self.scale = nn.Parameter(torch.ones(1))
        self.eps = 1e-8  # 避免数值不稳定
...
#             x = x + self.bias
        ...
        # 确保G是浮点类型
        if G.dtype != torch.float32 and G.dtype != torch.float64:
            G = G.float()
        ...
        # 应用可学习的缩放因子
        x = self.scale * torch.matmul(G_stable, x)
...
class HypergraphConv(MessagePassing):
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor,
        ...
        if hyperedge_index.numel() > 0:
            num_edges = int(hyperedge_index[1].max()) + 1
            # num_edges = int(hyperedge_index[1].unique().numel())

        # 确保size参数正确
        size_source_to_target = (num_nodes, num_edges)
        size_target_to_source = (num_edges, num_nodes)

        if hyperedge_weight is None:
            hyperedge_weight = x.new_ones(num_edges)

        x = x.to(self.weight.device)
        hyperedge_index = hyperedge_index.to(self.weight.device)
        hyperedge_weight = hyperedge_weight.to(self.weight.device)
        ...
        if not self.symdegnorm:
            ...
            out = self.propagate(hyperedge_index, x=x, norm=B, alpha=alpha,
                                 size=size_source_to_target)
            self.flow = 'target_to_source'
            out = self.propagate(hyperedge_index, x=out, norm=D, alpha=alpha,
                                 size=size_target_to_source)
